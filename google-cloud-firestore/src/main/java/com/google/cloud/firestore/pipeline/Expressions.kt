...
internal fun exprToValue(expr: Expr): Value{
  return when(expr) {
    ...
    is ListOfExprs -> {
      Value.newBuilder().setArrayValue(ArrayValue.newBuilder().addAllValues(expr.conditions.map { exprToValue(it) })).build()
    }
    else -> {
      TODO()
    }
...
interface Expr {
  ...
  fun inAny(vararg other: Any) = In(this, other.toList().map { when(it) {
        is Expr -> it
        else -> Constant.of(it) }})
  fun notInAny(vararg other: Any) = Function.Not(In(this, other.toList().map { when(it) {
        is Expr -> it
        else -> Constant.of(it) }}))
  ...
  fun arrayContainsAny(vararg elements: Any) = ArrayContainsAny(this, elements.toList().map { Constant.of(it) })
...
data class AggregatorTarget internal constructor(
  val accumulator: Function.Accumulator, val fieldName: String,
...
sealed class Function(val name: String, val params: List<Expr>): Expr {
  ...
  data class And<T>(val conditions: List<T>) :
    Function("and", conditions), FilterCondition where T : FilterCondition, T:Expr

  data class Or<T>(val conditions: List<T>) :
    Function("or", conditions), FilterCondition where T : FilterCondition, T:Expr
  ...
  companion object {
    ...
    @JvmStatic
    fun inAny(left: Expr, values: List<Any>) = In(left, values.map {
      when (it) {
        is Expr -> it
        else -> Constant.of(it)
      }
    ...
    @JvmStatic
    fun notInAny(left: Expr, values: List<Any>) = Not(In(left, values.map {
      when (it) {
        is Expr -> it
        else ->Constant.of(it)
      }}))
    ...
    @JvmStatic
    fun <T> and(left: T, right: T) where T : FilterCondition, T:Expr =
    ...
    @JvmStatic
    fun <T> or(left: T, right: T) where T : FilterCondition, T:Expr =
    ...
    @JvmStatic
    fun min(expr: Expr) = Sum(expr, false)

    @JvmStatic
    fun max(expr: Expr) = Avg(expr, false)

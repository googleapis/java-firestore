...
final class ServerSideTransaction extends Transaction {
  ...
  ApiFuture<Void> rollback() {
    ...
    try (TraceUtil.Scope ignored = span.makeCurrent()) {
      ...
      ApiFuture<Void> result = ApiFutures.catching(
              transform,
              Throwable.class,
              (error) -> {
                LOGGER.log(
  ...
  public ApiFuture<List<DocumentSnapshot>> getAll(
    ...
    try (TraceUtil.Scope ignored = span.makeCurrent()) {
      ApiFuture<List<DocumentSnapshot>> result = firestore.getAll(
  ...
  public ApiFuture<List<DocumentSnapshot>> getAll(
    ...
    TraceUtil.Span span =
            getTraceUtil()
                    .startSpan(TraceUtil.SPAN_NAME_TRANSACTION_GET_DOCUMENTS, transactionTraceContext);
    try (TraceUtil.Scope ignored = span.makeCurrent()) {
      ApiFuture<List<DocumentSnapshot>> result = firestore.getAll(documentReferences, fieldMask, transactionId, /*readTime=*/ null);
      span.endAtFuture(result);
      return result;
    } catch (Exception error) {
      span.end(error);

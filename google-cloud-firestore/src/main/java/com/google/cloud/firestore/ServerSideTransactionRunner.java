...
final class ServerSideTransactionRunner<T> {
  ...
  ApiFuture<ServerSideTransaction> begin() {
    TraceUtil.Span span =
            getTraceUtil().startSpan(TraceUtil.SPAN_NAME_TRANSACTION_BEGIN, transaction.getTransactionTraceContext());
    try (Scope ignored = span.makeCurrent()) {
      ServerSideTransaction previousTransaction = this.transaction;
      this.transaction = null;
      ApiFuture<ServerSideTransaction> result = ServerSideTransaction.begin(firestore, transactionOptions, previousTransaction);

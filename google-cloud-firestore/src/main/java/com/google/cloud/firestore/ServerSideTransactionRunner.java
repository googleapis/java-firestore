...
final class ServerSideTransactionRunner<T> {
  ...
  ApiFuture<ServerSideTransaction> begin() {
    TraceUtil.Span span = getTraceUtil().startSpan(TraceUtil.SPAN_NAME_TRANSACTION_BEGIN);
    try (Scope ignored = span.makeCurrent()) {
      ...
      ApiFutures.addCallback(
          result,
          new ApiFutureCallback<ServerSideTransaction>() {
            @Override
            public void onFailure(Throwable throwable) {}

            @Override
            public void onSuccess(ServerSideTransaction serverSideTransaction) {
